...
$dssCases = @(
  ...
  @("DSS-TC-009", "Donation Gate", "Donation selected with uniform branding is rejected", "User is logged in and chooses Donate.", "service_type=donate; item_type=Outerwear; condition=Good; cleanliness=Clean; details.uniform_branding=Yes", "Complete donation submission with uniform branding and open DSS confirmation.", "Donation is blocked and DSS returns rejected with message recommending Upcycle or Recycle.", "rejected", "getDonationBlock donation_uniform_branding", "Critical", "Functional", "", "", ""),
  ...
  @("DSS-TC-014", "Recycle Scoring", "Synthetic heavily damaged textile recommends Recycle", "User is logged in.", "service_type=recycle; item_type=Scraps; condition=Heavily damaged; cleanliness=Needs cleaning; fiber=polyester; fiber_composition=polyester_synthetic; damage=large_tear_heavy_damage; repurposing=low; trim_removal=easy; quantity=3", "Submit and open DSS confirmation.", "Recycle ranks first with high confidence.", "recycle", "Recycle weighted scoring for synthetic material and material recovery", "High", "Functional", "", "", ""),
  @("DSS-TC-015", "Recycle Scoring", "Heavily damaged big fabric panel with low repurposing ranks Recycle", "User is logged in.", "item_type=Big fabric panels (curtains, bedsheets); condition=Heavily damaged; cleanliness=Needs cleaning; fiber_composition=polyester_synthetic; repairability=not_practical; repurposing=low; trim_removal=none", "Complete form and review recommendations.", "Recycle appears as top recommendation.", "recycle", "Recycle condition/cleanliness/material/damage checks", "High", "Functional", "", "", ""),
  ...
  @("DSS-TC-017", "Upcycle Scoring", "Clean heavily damaged curtains with high repurposing recommend Upcycle", "User is logged in.", "service_type=upcycle; item_type=Big fabric panels (curtains, bedsheets); condition=Heavily damaged; cleanliness=Clean; fiber_composition=cotton_natural; wearability=not_wearable_fabric_usable; repairability=moderate_repair; repurposing=high; trim_removal=easy", "Submit and open DSS confirmation.", "Upcycle ranks first with high confidence.", "upcycle", "Upcycle weighted scoring for high repurposing potential", "High", "Functional", "", "", ""),
  @("DSS-TC-018", "Upcycle Scoring", "Scraps-only skips wearability and repairability", "User is logged in.", "item_types=Scraps only; condition=Minor damage; cleanliness=Clean; fiber_composition=cotton_natural; damage=small_hole_tear; contamination=clean; repurposing=high; trim_removal=easy", "Complete scraps submission and inspect DSS explanation checks.", "Wearability and Repairability show skipped/not applicable and are not required or scored.", "upcycle", "isFabricScrapsOnly skip scoring normalization", "High", "Boundary", "", "", ""),
  @("DSS-TC-019", "Upcycle Scoring", "Minor damage clean clothing with high repurposing recommends Upcycle", "User is logged in.", "item_type=Clothes (top, outerwear, bottoms); condition=Minor damage; cleanliness=Clean; fabric=denim/cotton; repairability=minor_repair; repurposing=high", "Submit and review DSS options.", "Upcycle should rank above Recycle and Donate when fabric value remains high.", "upcycle", "Upcycle condition + repair/redesign checks", "High", "Functional", "", "", ""),
  @("DSS-TC-020", "Buyback Preference", "Buyback interest is captured as partner handoff context", "User chooses or switches final DSS send pathway to Upcycle.", "final selected pathway=upcycle; buyback preference=Yes; upcycle_request='tote bag'; otherwise valid upcycle inputs", "Submit, choose Upcycle on DSS confirmation if needed, select Yes for buyback, then open partner request modal.", "Buyback is not a DSS recommendation. Upcycle confirmation and partner brief show buyback preference Yes and the requested upcycle outcome, even when the original pathway was Donate or Recycle.", "Upcycle, with buyback preference noted", "buyback_interest handoff branch", "Medium", "Functional", "", "", ""),
  @("DSS-TC-021", "Buyback Preference", "No buyback interest keeps handoff as upcycle only", "User selects Upcycle but buyback interest is No.", "buyback_interest=false", "Submit and inspect recommendation list and send confirmation.", "Buyback is not listed among DSS recommendations. Confirmation states Upcycle only, without buyback interest.", "Upcycle only", "buyback_interest false handoff branch", "Medium", "Functional", "", "", ""),
...
$negativeCases = @(
  ...
  @("DSS-NEG-002", "DSS API Validation", "Send DSS request with invalid pathway", "User is authenticated.", "recommended_pathway='compost'", "Call POST /api/dss/send.", "API rejects request because pathway is not recycle/donate/upcycle.", "N/A", "sendDssRecommendationSchema enum", "Critical", "Negative/API", "", "", ""),
...
$ruleMatrix = @(
  ...
  @("RULE-012", "Buyback Preference", "buyback_interest", "true", "Does not add a DSS score; appears only in upcycle confirmation and partner handoff brief", "upcycle context", "buildPartnerBrief"),
  @("RULE-013", "Scraps Skip", "details.item_types or item_type", "Scraps is the only selected item type", "Wearability and repairability are skipped and weights removed from scoring", "Recycle/Upcycle normalized score", "isFabricScrapsOnly; pathwayRuleScore"),
...
$summary = @(
  ...
  @("Pathways Covered", "recycle, donate, upcycle, rejected", "Includes eligibility gates; buyback is a yes/no upcycle preference, not a scored pathway", "buildPathwayRecommendations"),
