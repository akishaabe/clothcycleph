...
export function NotificationsPage() {
  ...
  const markAsRead = async (notification) => {
    ...
    try {
      ...
      if (ruleChangeRequestId) {
        if (notificationsTheme === "admin") {
          navigate(`/admin?panel=rule-requests&request=${ruleChangeRequestId}&highlight=${ruleChangeRequestId}`);
          return;
        }

        if (notificationsTheme === "partner") {
          navigate(`/partner?panel=rule-requests&highlight=${ruleChangeRequestId}`);
          return;
        }
      ...
      if (notification.data?.transactionId && notificationsTheme === "partner") {
        navigate(`/partner?request=${notification.data.transactionId}&highlight=${notification.data.transactionId}`);
        return;
      }

      if (notification.data?.transactionId) {
        navigate(`/dss-requests?request=${notification.data.transactionId}&highlight=${notification.data.transactionId}`);
        return;
      }

      if (notification.data?.submissionId) {
        navigate(`/dss/${notification.data.submissionId}?highlight=${notification.data.submissionId}`);
        return;
      }
