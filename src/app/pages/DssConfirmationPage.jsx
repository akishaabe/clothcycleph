...
function formatCheckList(checks = [], matched) {
  const labels = checks
    .filter((check) => Boolean(check.matched) === matched)
    .map((check) => check.question);

  if (labels.length === 0) {
    return matched ? "No strong matches recorded." : "No major misses recorded.";
  }
...
function formatRecommendationScores(recommendations = []) {
  if (!recommendations.length) {
    return "No pathway scores available.";
  }

  return recommendations
    .map((recommendation) => {
      const label = pathwayLabels[recommendation.recommended_pathway] || recommendation.recommended_pathway;
      const confidence = Math.round(Number(recommendation.confidence || 0) * 100);
      const score = recommendation.score != null ? Number(recommendation.score).toFixed(1) : "N/A";
