import { D1Database, queryD1 } from '../config/d1.js';
...
export async function listPartnerLocationsD1(db: D1Database, options: PartnerSearchOptions = {}) {
  ...
  const partners = (result.results ?? [])
    .map((partner: any) => enrichPartner(partner, options))
    .filter((partner: any) => {
